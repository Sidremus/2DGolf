...
func update_UI(delta: float):
	...
	if is_shooting && (is_selected || !Game.multi_ball_mode):
		...
		aim_line.position = aim_line_vec.normalized() * Game.max_mouse_line_length * .05
		power_line.points[1].y = -(mouse_line.points[1].length() / Game.max_mouse_line_length) * 125
	...
	if is_shooting && (is_selected || !Game.multi_ball_mode):
		...
		t = "\n\nCurrent:\n  Power: " + str(snappedf(power_lerp * 100., 1.)).pad_decimals(1) + "%"
		t = t + "\n  Angle: " + str(snappedf(rad_to_deg(angle + PI), .1)).pad_decimals(1) + "°"
	else: t = "\n\nCurrent:\n  Power:  -  %\n  Angle:  -  °"
	current_shot_info_label.text = t
	if Game.hovering_shot_info: current_shot_info_label.visible = is_shooting && (is_selected || !Game.multi_ball_mode)
...
func finish_shot():
	...
	var shot_vec:Vector2 = -mouse_line.points[1].normalized() * Game.shot_power * power_lerp
	if Game.reset_velocity_on_shot: linear_velocity = shot_vec if !Game.reverse_mouse_draw else -shot_vec
	else: apply_central_impulse(shot_vec if !Game.reverse_mouse_draw else -shot_vec)
...
func _integrate_forces(_state: PhysicsDirectBodyState2D) -> void:
	...
	ball_sprite.modulate = Color.WHITE if can_shoot else Color.WEB_GRAY
...
func _ready() -> void:
	...
	if !Game.hovering_shot_info: current_shot_info_label.global_position = Vector2(260, 50.)
	else: current_shot_info_label.hide()
	ball_impact_volume_ref = ball_impact_AS.volume_db
	
	start_marker.top_level = true
	start_marker.global_position = global_position
	start_marker.show()
